...
import BiddingLaunchForm from "@/components/bidding/BiddingLaunchForm"; // 👈 Import the new form
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Timer, Gavel, StopCircle, ArrowRight } from "lucide-react";
...
export default async function BiddingPage() {
  ...
  // 3. NEW: Get ENDED Sessions (History)
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-brand-highlight flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-gray-500 font-medium ml-9">
            Start auctions and monitor bids.
          </p>
        </div>
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          {liveSessions.length === 0 ? (
            ...
            liveSessions.map((session) => (
              <div
                key={session.id}
                className="bg-white p-5 rounded-2xl shadow-sm border border-brand-highlight flex flex-col md:flex-row justify-between items-center gap-4"
              >
                {/* Details */}
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500 font-medium flex items-center gap-1">
                      {session.order.fromLocation}{" "}
                      <ArrowRight className="h-3 w-3" />{" "}
                      {session.order.toLocation}
                    </div>
                    <div className="mt-1 inline-flex items-center gap-2 bg-gray-100 px-2 py-0.5 rounded text-[10px] font-bold text-gray-600">
                      Base: ₹{session.basePrice.toLocaleString()} •{" "}
                      {session._count.bids} Bids Received
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-10 pt-10 border-t border-slate-200">
                  <h3 className="font-bold text-slate-400 text-sm uppercase tracking-wider mb-4">
                    Stopped / Ended Auctions
                  </h3>
                  ...
                  <div className="space-y-3">
                    ...
                    {historySessions.map((session) => (
                      <div
                        key={session.id}
                        className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex justify-between items-center opacity-75 hover:opacity-100 transition"
                      >
                        <div>
                        ...
                        <form
                          action={async () => {
                            "use server";
                            await import("@/actions/bidding-actions").then(
                              (mod) => mod.repostAuctionAction(session.id),
                            );
                          }}
                        >
                          <button className="text-xs font-bold bg-white border border-slate-300 px-3 py-1.5 rounded-lg shadow-sm hover:bg-blue-50 hover:text-blue-600 transition flex items-center gap-2">
                            🔄 Repost Auction
                          </button>
                        </form>
                      </div>
